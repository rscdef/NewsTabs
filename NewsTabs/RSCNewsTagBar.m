...
@implementation RSCNewsTagInfo

@end

@interface RSCNewsTagBar() {
...
@implementation RSCNewsTagBar

+ (RSCNewsTagBar *)createTabBar {
    return [[NSBundle mainBundle] loadNibNamed:@"RSCNewsTagBar" owner:nil options:nil][0];
}
...
- (void)setupWithTagInfos:(NSArray *)tagInfos {
    ...
    for (RSCNewsTagInfo *tag in tagInfos) {
...
- (void)scrollToCurrentIndex {
    RSCNewsTagInfo *tagInfo = _tagInfos[_lastIndex];
    ...
    RSCNewsTagInfo *lastTagInfo = [_tagInfos lastObject];
